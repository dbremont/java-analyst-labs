...
async def get_time_distribution():
    # Group by Minute substring
    query = '''
        SELECT substr(timestamp, 1, 16) as bucket_time, count(*) as count
        FROM logs
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(query)
        data = [{"time": row['bucket_time'], "count": row['count']} for row in cur.fetchall()]
    return {"data": data}

@app.get("/api/analytics/word-cloud")
async def get_word_cloud(limit: int = 50):
    # Query the FTS5 Vocabulary table directly
    # Note: table name usually 'logs_fts_vocab' when using default config
    with get_db() as conn:
        cur = conn.cursor()
        try:
            cur.execute(f'''
